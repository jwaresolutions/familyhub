...
export function KanbanColumn({ status, label, tasks, onTaskClick, onCompleteTask }: KanbanColumnProps) {
    ...
    <div className="flex flex-col w-72 min-w-[18rem] shrink-0">
      ...
      >
        <SortableContext items={tasks.map(t => t.id)} strategy={verticalListSortingStrategy}>
          {tasks.map(task => (
            <TaskCard key={task.id} task={task} onClick={() => onTaskClick(task)} onComplete={onCompleteTask ? () => onCompleteTask(task.id) : undefined} />
          ))}
        </SortableContext>
      </div>
    </div>
